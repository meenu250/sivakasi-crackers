...
router.post('/:type', (req, res) => {
  ...
  const calcPrice = mrp && discount ? parseFloat(mrp)*(1-parseFloat(discount)/100) : parseFloat(price);
  const calcDiscount = mrp && price ? Math.round(((parseFloat(mrp)-parseFloat(price))/parseFloat(mrp))*100) : parseFloat(discount||0);
  const item = { id: `col_${nextId++}`, name, desc: desc||'', price: parseFloat(calcPrice.toFixed(2)), mrp: parseFloat(mrp||price), discount: calcDiscount, stock: parseInt(stock), lowStockThreshold: parseInt(lowStockThreshold||10), image: image||'', createdAt: new Date().toISOString() };
  list.unshift(item);
...
router.put('/:type/:id', (req, res) => {
  ...
  const newMrp   = parseFloat(mrp  || oldItem.mrp);
  const newPrice = mrp && discount  ? newMrp*(1-parseFloat(discount)/100) : parseFloat(price || oldItem.price);
  const newDisc  = newMrp>newPrice  ? Math.round(((newMrp-newPrice)/newMrp)*100) : 0;
  list[idx] = { ...oldItem, name: name||oldItem.name, desc: desc??oldItem.desc, price: parseFloat(newPrice.toFixed(2)), mrp: newMrp, discount: newDisc, stock: parseInt(stock??oldItem.stock), lowStockThreshold: parseInt(lowStockThreshold??oldItem.lowStockThreshold??10), image: image!==undefined?image:oldItem.image };
  res.json(list[idx]);
});
